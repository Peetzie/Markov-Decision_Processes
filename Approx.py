...
def semi_gradient_n_step_sarsa(value_function, n=1):
    ...
    while True:
        ...
        if time < T:
            ...
            new_position, new_velocity, reward = step(
        ...
        if update_time >= 0:
            ...
            if update_time + n <= T:
                returns += value_function.value(
                    positions[update_time + n],
...
def figure_10_1():
    ...
    axes = [
        fig.add_subplot(1, len(plot_episodes), i + 1, projection="3d")
        for i in range(len(plot_episodes))
    ]
    num_of_tilings = 20
    alpha = 0.3
    value_function = ValueFunction(alpha, num_of_tilings, max_size=2048 * 100)
    ...
    g = GENV(10)
    g.createResDir(images=True)
    g.save_img(img=fig, name="Approxmated")
    plt.close()
...
def figure_10_2():
    ...
    for i in range(0, len(alphas)):
        fig = plt.plot(
            steps[i], label="alpha = " + str(alphas[i]) + "/" + str(num_of_tilings)
        )
    plt.xlabel("Episode")
